...
//| Custom function                                                  |
//+------------------------------------------------------------------+
//Custom Function - Gets ATR value
double GetATRValue()
{
   ...
   bool FillAtr = CopyBuffer(HandleAtr,IndexAtr,0,3,BufferAtr); //Copy buffer uses oldest as 0 (reversed)
...
bool ProcessTradeOpen(ENUM_ORDER_TYPE OrderType, double CurrentATR)
{
   ...
   if(OrderType == ORDER_TYPE_BUY)
   {
      ...
      StopLossPrice   = NormalizeDouble(Price - AtrLossMulti*CurrentATR,   Digits());
      TakeProfitPrice = NormalizeDouble(Price + AtrProfitMulti*CurrentATR, Digits());
   }
