...
//| SL uses Indicator F1 as on chart indicator. TP from Risk Reward. |
//| Position size is calculated using a percentage of account. User  |
//| can select either fixed or compounding position sizing.          |
//+------------------------------------------------------------------+
   ...
   for(int SymbolLoop=0; SymbolLoop < NumberOfTradeableSymbols; SymbolLoop++)
   {
      ...
      if(TimeLastTickProcessed[SymbolLoop] != iTime(CurrentSymbol,Period(),0))
      {
         ...
         )
         {
            ...
            StopLossPrice      = GetIndicatorValue(IndicatorHandleF1[SymbolLoop], 0); 
            TakeProfitPrice    = GetTakeProfit(CurrentSymbol, OrderType, EntryPrice, StopLossPrice, RiskReward);
